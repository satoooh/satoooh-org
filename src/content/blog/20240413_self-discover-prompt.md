...
他にも、Least-to-Most Prompting という手法 [^zhou2023leasttomost] では、問題を小さな部分問題に分割し、それらを順に解くように指示する手法で、「**困難は分割せよ**」というデカルトの考え方に通じるものがあります。
...
といったように、プロンプトに関する手法は数多く提案されているのですが、2024年2月に arXiv に投稿された **"SELF-DISCOVER: Large Language Models Self-Compose Reasoning Structures"** [^zhou2024selfdiscover] という論文で提案された手法が興味深かったので、この記事ではその手法を実装・検証した内容をまとめます。
...
Stage 1では、**推論モジュール RM (Reasoning Module) と SELECT、ADAPT、IMPLEMENT の3つのステップを通じて、タスクに適した推論構造を発見します**。

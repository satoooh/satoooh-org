...
和文フォントが重いことが原因として想定されるのですが、フォント読み込みで利用している @next/font が Next.js v13（2022 年 10 月リリース） で追加された比較的新しい技術であるため、あまりインターネット上に知見がありませんでした。
...
この設定では、Web フォントのダウンロードに時間がかかるとき、次の読み込みまではフォールバックフォントを表示します。
